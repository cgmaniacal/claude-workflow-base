...
**Status:** accepted (database changed from MySQL to PostgreSQL in ADR-004)
**Date:** 2026-02-10
**Deciders:** Project maintainer

## Context

Full-stack projects require a database for local development. Previously, the boilerplate assumed developers would install the database natively on their machine. This creates several issues:

- Database installation varies by OS (Homebrew on macOS, apt on Linux, installer on Windows)
...
- Developers must manage a system-wide database process alongside other projects

The alternative approaches considered:
1. **Native database installation** (previous approach) — simplest, but fragile and OS-dependent
...
- PostgreSQL 16 container with health check (see ADR-004 for database choice)
...
- Zero database installation required — Docker Desktop is the only prerequisite
